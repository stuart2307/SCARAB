    ...
    while 1:
        ...
        match int(choice):
            ...
            case 4:
                ...
                romSize =1024*(1 << header[23])
                print(romSize)
                gameName = re.sub(r'[<>:"/\\|?*\x00-\x1F]', '_', currentRom.strip())
                filename = gameName + ".rom"
                rom = open(filename, "wb")
                buffer = scarab.read_until("SCARABROMDUMPTRUSTMEBRO",size=romSize)
                rom.write(buffer)
                rom.close()
                print("Buffer in!")
                print("Saved as " + filename)
